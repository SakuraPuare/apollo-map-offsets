...
def create_scenario_from_data(scenarios_template: dict, data_objects: List[dict],
    ...
    # 如果提供了匹配结果，只处理匹配的障碍物
    if match_results:
        matched_data_ids = set(pair['dst_id'] for pair in match_results['matched_pairs'])
        print(f"使用匹配结果，只保留 {len(matched_data_ids)} 个匹配的障碍物")
        objects_to_process = [obj for obj in data_objects if str(obj['id']) in matched_data_ids]
    else:
...
def main():
    ...
    # 保存
    print(f"\n保存新场景: {args.output}")
    save_json(new_scenario, args.output)
