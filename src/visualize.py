...
def visualize_matching_overview(
    results: dict, output_file: str = "visualizations/01_matching_overview.png"
):
    """生成匹配总览图（6子图）"""
    ...
    src_points = np.array(
        [[p["src_pos"]["x"], p["src_pos"]["y"]] for p in matched_pairs]
    )
    dst_points = np.array(
        [[p["dst_pos"]["x"], p["dst_pos"]["y"]] for p in matched_pairs]
    )
    ...
    for i in range(len(src_shifted)):
        ax2.plot(
            [src_shifted[i, 0], dst_points[i, 0]],
    ...
    set_chinese_labels(
        ...
        xlabel="X (m)",
        ylabel="Y (m)",
        title=f"应用偏移后 (Δx={dx:.2f}m, Δy={dy:.2f}m)",
    )
    ...
    ax5.scatter(
        offsets[:, 0], offsets[:, 1], c="purple", s=100, alpha=0.6, edgecolors="black"
    )
    ax5.axvline(
        dx, color="red", linestyle="--", linewidth=2, label=f"Mean Δx={dx:.2f}m"
    )
    ax5.axhline(
        dy, color="blue", linestyle="--", linewidth=2, label=f"Mean Δy={dy:.2f}m"
    )
...
def visualize_vector_field(
    results: dict, output_file: str = "visualizations/02_vector_field.png"
):
    """可视化偏移向量场"""
    matched_pairs = results["matched_pairs"]
    src_points = np.array(
    ...
    ax.set_title(
        "障碍物变换向量场", fontproperties=font_props, fontsize=16, fontweight="bold"
    )
...
def visualize_obstacles_comparison(
    ...
    plt.suptitle(
        "障碍物场景对比", fontproperties=font_props, fontsize=18, fontweight="bold"
    )
    plt.tight_layout()
...
def generate_all_visualizations(
    ...
    visualize_obstacles_comparison(
        scenarios_obs, data_obs, results, f"{output_dir}/03_obstacles_comparison.png"
    )
...
def main():
    ...
    parser.add_argument(
        "--results", default="results/offset_results.json", help="匹配结果文件"
    )
    parser.add_argument(
        "--scenarios", default="input/scenarios.json", help="Scenarios文件"
    )
    parser.add_argument("--data", default="input/data.json", help="Data文件")
