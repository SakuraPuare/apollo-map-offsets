...
def load_offset_from_results(results_file: str = 'results/offset_results.json') -> tuple:
    """
    从结果文件加载偏移量
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='对 Apollo HD Map 应用偏移变换')
    parser.add_argument('input_map', type=str,
                        help='输入地图文件路径 (text format .txt 或 binary .bin)')
    parser.add_argument('output_map', type=str,
                        help='输出地图文件路径')
    parser.add_argument('--offset-file', type=str,
                        default='results/offset_results.json',
                        help='偏移结果文件路径（JSON格式）')
    parser.add_argument('--offset-x', type=float,
                        help='手动指定X偏移量（米），会覆盖文件中的值')
    parser.add_argument('--offset-y', type=float,
                        help='手动指定Y偏移量（米），会覆盖文件中的值')
    parser.add_argument('--rotation', type=float,
                        help='手动指定旋转角度（弧度），会覆盖文件中的值')
    parser.add_argument('--format', type=str, choices=['text', 'binary'],
                        default='text',
