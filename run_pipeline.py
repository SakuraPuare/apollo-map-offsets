...
示例:
  ...
  python3 run_pipeline.py --all-objects
    ...
    parser.add_argument(
        "--map-file", type=str, help="地图文件路径 (默认: 自动从 scenarios.json 提取)"
    )

    parser.add_argument(
        "--skip-map", action="store_true", help="跳过地图处理，只处理场景"
    )
    ...
    parser.add_argument(
        "--skip-decrypt", action="store_true", help="跳过解密步骤 (即使 raw.json 存在)"
    )
...
╚═══════════════════════════════════════════════════════════════════╝
    ...
    if (
        ...
        and Path("input/raw.json").exists()
        and not Path("input/data.json").exists()
    ):
        print_step(0, "解密原始数据")
        if not run_command(
            ["python3", "src/step0_decrypt_raw_data.py"], "解密 raw.json"
        ):
            return 1
    ...
    if not run_command(["python3", "src/step1_calculate_offset.py"], "计算偏移量"):
        return 1

    # Step 2: 生成可视化
    print_step(2, "生成可视化图表")
    if not run_command(["python3", "src/visualize.py"], "生成所有可视化图表"):
        return 1
    ...
    # 获取生成的场景ID和mapId（从输出文件中读取）
    import glob

    scenario_files = sorted(
        glob.glob("output/*.json"), key=lambda x: Path(x).stat().st_mtime, reverse=True
    )
    new_map_id = None
    if scenario_files:
        ...
        try:
            with open(scenario_output, "r") as f:
                scenario_data = json.load(f)
                new_map_id = scenario_data.get("mapId")
                if new_map_id:
                    print(f"   新地图 ID: {new_map_id}")
        except Exception:
            pass
    else:
    ...
    if has_map:
        ...
        map_cmd = [
            "python3",
            "src/step2_apply_offset_to_map.py",
            map_file,
            str(output_map_file),
            "--format",
            "binary",
        ...
        if new_map_id:
            map_cmd.extend(["--new-map-id", new_map_id])

        if not run_command(map_cmd, "应用偏移到地图"):
            return 1
    ...
    if has_map:
        print("  2. 复制地图到 Apollo:")
        print(
            f"     cp -r output/{map_name}_offset /apollo_workspace/modules/map/data/"
        )
        print("  3. 在 Dreamview 中:")
