...
def estimate_initial_transform(src_obs: List[Obstacle], dst_obs: List[Obstacle]) -> Tuple[float, float]:
    """
...
def match_obstacles_hungarian(src_obs: List[Obstacle], dst_obs: List[Obstacle],
                               initial_offset: Tuple[float, float] = None,
                               max_distance: float = 50.0,
                               dimension_weight: float = 100.0) -> List[Tuple[int, int, float]]:
    """
    使用匈牙利算法匹配障碍物
    ...
    for i, src in enumerate(src_obs):
        ...
        for j, dst in enumerate(dst_obs):
            # 坐标距离
            pos_dist = np.sqrt((src_x_adjusted - dst.x)**2 + (src_y_adjusted - dst.y)**2)

            # 尺寸差异（L1距离）
            dim_diff = (abs(src.length - dst.length) +
...
def calculate_transform_from_matches(src_obs: List[Obstacle], dst_obs: List[Obstacle],
...
def main():
    ...
    print(f"\n简单平移统计（未考虑旋转）:")
    print(f"  dx: mean={result['offsets'][:, 0].mean():.2f}, std={result['offsets'][:, 0].std():.2f}")
    print(f"  dy: mean={result['offsets'][:, 1].mean():.2f}, std={result['offsets'][:, 1].std():.2f}")

    # 显示前20个匹配
    print(f"\n前20个匹配点对:")
    print(f"{'No.':<4} {'Src ID':<10} {'Dst ID':<10} {'Cost':<10} {'Error(m)':<10} {'Src Pos':<25} {'Dst Pos':<25}")
    print("-" * 110)
    for i, (src_idx, dst_idx, cost) in enumerate(matches[:20]):
        ...
        print(f"{i+1:<4} {src.id:<10} {dst.id:<10} {cost:<10.4f} {error:<10.4f} {src_pos:<25} {dst_pos:<25}")
